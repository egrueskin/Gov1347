...
ggplot(data=all_2%>% filter(MidtermYear==1),aes(x=approval,y=majorvote_pct,label=year))+
...
lm_seats<-lm(seats~Inc_party_pres*(MidtermYear+approval+GDP_growth_pct)+poll_pct+lag_pv,data=train)
...
# load polling data
dist_polls_2018_2022 <- read_csv("/Users/elliegrueskin/Documents/Fall 2022/Gov 1347/Gov1347/data/by_district/dist_polls_2018-2022.csv")
...
# load data
incumb_df <- read_csv("/Users/elliegrueskin/Documents/Fall 2022/Gov 1347/Gov1347/data/by_district/incumb_dist_1948-2020.csv")
...
oh <- left_join(oh, polls_df)%>%
  filter(party!='LIB')%>%
  mutate(VoteShare=ifelse(party=='REP',RepVotesMajorPercent,DemVotesMajorPercent),
         Incumbent=ifelse(party=='REP'&RepStatus=='Incumbent',1,0))

# fit lm
dat_poll_inc <- oh[oh$Incumbent==1,]

#dat_poll_chl <- oh[!oh$INCUMB,]
mod_poll_inc <- lm(VoteShare ~ pct, data = dat_poll_inc)

...
ggplot(test,aes(x=seats,pred_seats[,"fit"],label=year,col=party))+geom_point()+geom_smooth(method=lm,se=FALSE)+geom_label(title='Predicted Seats vs. Actual Seats')+scale_color_manual(values=c("Blue","Red"))
ggplot(test,aes(x=majorvote_pct,pred_vote[,"fit"],label=year,col=party))+geom_point()+geom_smooth(method=lm,se=FALSE)+geom_label(title='Predicted Voteshare vs. Actual Voteshare')+scale_color_manual(values=c("Blue","Red"))
